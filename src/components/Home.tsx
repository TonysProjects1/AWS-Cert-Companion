...
const CATEGORIES = ['Foundational', 'Associate', 'Professional', 'Specialty'] as const;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="space-y-16">
          {CATEGORIES.map(category => {
            const certsInCategory = CERT_OPTIONS.filter(cert => cert.category === category);
            if (certsInCategory.length === 0) return null;

            return (
              <div key={category} className="space-y-6">
                <h2 className="text-sm font-bold uppercase tracking-[0.2em] opacity-50 border-b border-[#1A1A1A]/10 pb-2">
                  {category}
                </h2>
